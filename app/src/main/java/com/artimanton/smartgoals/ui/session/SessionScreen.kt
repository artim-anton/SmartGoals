...
import com.artimanton.smartgoals.ui.theme.Red
...
private fun SessionScreen(
    ...
    LaunchedEffect(key1 = state.goals) {
        val goalId = timerService.goalId.value
        onEvent(
            SessionEvent.UpdateGoalIdAndRelatedGoal(
                goalId = goalId,
                relatedToGoal = state.goals.find { it.goalId == goalId }?.name
            )
        )
    }
